<template>
  <div class="min-h-screen bg-gray-50">
    ...
    >
      <div class="bg-white w-full max-w-md h-full shadow-2xl flex flex-col overflow-hidden animate-slide-in">
        ...
        <div v-if="cart.length === 0" class="flex-1 flex flex-col justify-center items-center text-center p-8">
        ...
        <div v-else class="flex-1 overflow-y-auto p-6 space-y-5">
          <div v-for="item in cart" :key="item.id" class="flex gap-4 border border-gray-100 rounded-2xl p-4">
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center gap-3 mt-3">
                <button @click="changeQuantity(item.id, -1)" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-xl">-</button>
        ...
        <div v-if="cart.length > 0" class="p-6 border-t bg-white">
          <div class="flex justify-between text-xl mb-5">
            <span class="font-medium text-gray-600">Total Bill</span>
            <span class="font-black text-amber-600">${{ totalPrice.toFixed(2) }}</span>
          </div>
